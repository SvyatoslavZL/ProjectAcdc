...
VALUES ('Ivan', 'qwerty', 'ADMIN', '2024-02-02', 10),
...
SELECT login, password, role, game_point
FROM player
...
SELECT role, count(role) N FROM (SELECT login, role FROM player) GROUP BY role;

SELECT p.role, sum(p.game_point) points FROM player p
WHERE game_point > 0
GROUP BY role
HAVING sum(p.game_point) > 13
